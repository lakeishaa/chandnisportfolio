var colour = "#f83b8a"; // Change the color to the specified hex code
...
window.onload = function () {
  if (document.getElementById) {
    ...
    for (var i = 0; i < sparkles; i++) {
      var rats = createDiv(5, 5); // Adjusted size here
      rats.style.visibility = "hidden";
      rats.style.zIndex = "999";
      document.body.appendChild((tiny[i] = rats));
      starv[i] = 0;
      tinyv[i] = 0;
      var rats = createDiv(9, 9); // Adjusted size here
      rats.style.backgroundColor = "transparent";
      rats.style.visibility = "hidden";
      rats.style.zIndex = "999";
      var rlef = createDiv(1, 9); // Adjusted size here
      var rdow = createDiv(9, 1); // Adjusted size here
      rats.appendChild(rlef);
      rats.appendChild(rdow);
      rlef.style.top = "4px"; // Adjusted position here
      rlef.style.left = "0px";
      rdow.style.top = "0px";
      rdow.style.left = "4px"; // Adjusted position here
  ...
  if (Math.abs(x - ox) > 1 || Math.abs(y - oy) > 1) {
    ...
    for (c = 0; c < sparkles; c++)
      if (!starv[c]) {
        ...
        star[c].style.clip = "rect(0px, 9px, 9px, 0px)"; // Adjusted size here
...
function update_star(i) {
  if (--starv[i] == 25) star[i].style.clip = "rect(1px, 8px, 8px, 1px)"; // Adjusted size here
  if (starv[i]) {
